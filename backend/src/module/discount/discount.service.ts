...
export class DiscountService {
    ...
    async findAll(req: Request) {
        const { page = 1, limit = 5, sortBy = "id", orderBy = "ASC" } = req.query;
    ...
    async searchAndFilter(req: Request) {
        const { code = "%", page = 1, limit = 5, sortBy = "value", orderBy = "ASC" } = req.query;

        const take = isNaN(Number(limit)) ? 5 : Number(limit);
        const skip =
            isNaN(Number(page)) || isNaN(Number(limit)) ? 0 : (Number(page) - 1) * Number(limit);
        const sortByValid = Object.values(SortField).filter(
            (item) => item === (sortBy as string).toLowerCase()
        );
