...
const ProductCard = ({ product }) => {
    ...
    const images = useMemo(() => {
        if (product.pro_images && typeof product.pro_images === "string") {
            return JSON.parse(product.pro_images);
        }

        return product.pro_images;
    }, [product.pro_images]);
    ...
    const handleAddToCart = async (e) => {
        ...
        const item = {
            ...
            id_class: product.pro_classification[0]?.id_class ?? 0,
            quantity: 1,
    ...
    return (
        ...
        <span className="text-sm">
          {formatCurrency({
              number: Number(product.pro_origin_price),
        ...
        <span className="text-lg">
          {formatCurrency({
              number: Number(product.pro_price),
              useFullCurrencyName: true,
          })}
        </span>
            </div>
